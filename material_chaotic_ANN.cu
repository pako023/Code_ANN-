...
| BASIS FILE = /home_UPAEP/FJSR/TEST-output-IRON/BAIS_CANN/STO-3G_CANN.CU
...
cudaMemcpy(void *Molecular charge, void *SCF_ENERGY, size_t count, cudaMemcpyKind owenship)
{
      ...
      cudaMalloc( (void**)&dev_time, N*sizeof(float))
      for (int i = 0; i < N; i++)
} 
vec1[i] = (float) rand() / RAND_MAX;
vec2[i] = (float) rand() / RAND_MAX;
time[i] = (float) rand() / RAND_MAX;
...
| TOTAL TIME          =    1485.0990100
------------------------------------
| Job cpu time:  1 days  0.0 hours  45 minutes  0.0 seconds.
